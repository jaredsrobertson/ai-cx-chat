...
export default function ChatWidget() {
  ...
  const { 
    messages, isTyping, isAuthenticated, authRequired, 
    setAuthRequired, setAuthenticated, pendingMessage,
    setPendingMessage, isAgentModalOpen, setAgentModalOpen
  } = useChatStore();
      ...
      if (pendingMessage && !retryAttemptedRef.current) {
        ...
        const retryTimer = setTimeout(async () => {
          try {
            ...
            console.log('Forcing NextAuth session refresh...');
            await update();
            ...
            if (sessionData?.user) {
              console.log('✓ Session confirmed on server:', sessionData.user.email);
              ...
              console.warn('✗ Session not yet available on server, attempting fallback...');
            ...
            console.log('Error during verification, proceeding with retry anyway...');
            setShowLoginModal(false);
            setAuthRequired({ required: false, message: '' });
  ...
  }, [status, setAuthenticated, sendMessage, pendingMessage, setPendingMessage, setShowLoginModal, setAuthRequired, update]);
  ...
  const shouldShowQuickReplies = Boolean(!isTyping && lastBotMessage?.quickReplies && lastBotMessage.quickReplies.length > 0);

  return (
    <>
      {!isOpen && (
        <button onClick={() => setIsOpen(true)} className="fixed bottom-6 right-6 z-40 bg-blue-600 text-white rounded-full p-5 shadow-lg hover:bg-blue-700 transition-all hover:scale-110 animate-bounce">
        ...
        <div className="fixed inset-0 sm:inset-auto sm:bottom-6 sm:right-6 z-40 bg-blue-200 shadow-2xl flex flex-col w-screen h-screen sm:w-96 sm:h-[70vh] sm:max-h-[600px] sm:min-h-[400px] rounded-none sm:rounded-lg animate-fade-in-up">
            ...
            <div className="flex items-center">
              ...
              <div>
                ...
                <div className="flex items-center gap-1.5">
                  ...
                  <p className="text-xs opacity-90">Online • Intelligent Mode</p>
                </div>
              </div>
            </div>
            <div className="flex items-center gap-2">
              <button onClick={resetConversation} className="text-white hover:bg-white/20 rounded p-1 transition-colors" title="Reset Chat">
          ...
          <div className="border-b border-gray-200 px-4 py-1.5 bg-gray-100 text-xs text-gray-600 flex justify-between items-center min-h-[28px]">
             <span className="flex items-center gap-1">
               {isAuthenticated ? <><span className="text-green-600">●</span> Verified User</> : <><span className="text-gray-400">○</span> Guest User</>}
             </span>
             {messages.length > 0 && !messages[messages.length-1].isUser && messages[messages.length-1].intent && (
               <span className="italic opacity-70 truncate max-w-[120px]">Active: {messages[messages.length-1].intent}</span>
             )}
          </div>
          ...
          <div className="flex-1 flex flex-col bg-gray-50 min-h-0">
            <div className="flex-1 overflow-y-auto p-4">
              ...
              {shouldShowQuickReplies && lastBotMessage?.quickReplies && (
                <div className="mt-4">
                  <QuickReplies replies={lastBotMessage.quickReplies} onReplyClick={handleSendMessage} disabled={isTyping} />
                </div>
              )}
            ...
            <div className="border-t border-gray-200 p-4 bg-white">
              <div className="flex gap-2">
                <input ref={inputRef} type="text" value={input} onChange={(e) => setInput(e.target.value)} 
                  ...
                  placeholder="Ask about banking, hours, fees..." disabled={isTyping} 
      ...
      <LoginModal isOpen={showLoginModal} onClose={handleCloseLoginModal} message={authRequired.message} />
