...
import { logger } from './logger'; // Corrected logger import
...
if (mockMode) {
  logger.warn("OpenAI API key not found. 'AI Advisor' will run in mock mode.");
}

export async function classifyIntent(queryText) {
  if (mockMode) return 'None';
  ...
  try {
    ...
    logger.debug(`OpenAI classified intent`, { query: queryText, intent });
    
    return validIntents.includes(intent) ? intent : 'None';
  } catch (error) {
    logger.error("Error classifying intent with OpenAI", error);
    return 'None';
  }
}
...
export async function getFinancialAdviceStream(messages) {
  ...
  const systemPrompt = `
      You are a helpful AI customer experince chat bot for a corporate bank.
      Your goal is to answer questions and provide tips related to basic, general personal finance.

      Follow these rules strictly:
      1. Keep your entire response to a maximum of 30 words.
    ...
    const stream = new ReadableStream({
      async start(controller) {
        ...
        try {
          ...
          logger.error('Streaming error from OpenAI', streamError);
