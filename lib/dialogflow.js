...
  }
  ...
  // FIXED: This function now correctly and reliably reads from the context.
  handleTransferConfirmation(queryResult, userContext) {
    ...
    // For confirmation intents, the details are in the output context from the previous turn.
    // We look for the context that Dialogflow carries over.
    const transferContext = queryResult.outputContexts?.find(ctx => 
      ctx.name.includes('/contexts/account.transfer-followup')
    );
  
    // If the context exists, extract the parameters from it.
    ...
    // ONLY if we successfully found all details, create the confidential display.
    if (amount && fromAccount && toAccount) {
      ...
      // Otherwise, return the simple confirmation text. This was the previous behavior.
