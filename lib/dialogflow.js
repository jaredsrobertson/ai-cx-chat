...
class DialogflowService {
  ...
  handleTransferRequest(queryResult, userContext) {
    ...
    // STREAMLINED: Let Dialogflow handle parameter collection through slot filling
    const params = queryResult.parameters?.fields || {};
    ...
    console.log('Transfer params extracted:', { amount, fromAccount, toAccount });
    
    // If Dialogflow hasn't collected all required parameters, it will handle the prompting
    ...
    // This shouldn't happen if Dialogflow slot filling is working correctly
  ...
  handleTransferConfirmation(queryResult, userContext) {
    ...
    // STREAMLINED: Use the exact context names from your Dialogflow setup
    ...
    if (transferContext?.parameters?.fields) {
      ...
      // Handle missing destination account with smart default
