...
class DialogflowService {
  ...
  handleTransferConfirmation(queryResult, userContext) {
    ...
    // Attempt 1: Get params from the current intent's parameters
    const currentParams = queryResult.parameters.fields;
    amount = currentParams.amount?.structValue?.fields?.amount?.numberValue;
    fromAccount = currentParams.source_account?.stringValue;
    toAccount = currentParams.destination_account?.stringValue;

    // Attempt 2 (Fallback): If not found, look in the output contexts
    if ((!amount || !fromAccount || !toAccount) && queryResult.outputContexts) {
      const transferContext = queryResult.outputContexts.find(ctx => ctx.name.endsWith('/contexts/account.transfer-followup'));
      if (transferContext?.parameters?.fields) {
        const contextParams = transferContext.parameters.fields;
        amount = contextParams.amount?.structValue?.fields?.amount?.numberValue ?? amount;
        fromAccount = contextParams.source_account?.stringValue ?? fromAccount;
        toAccount = contextParams.destination_account?.stringValue ?? toAccount;
      }
    }

    if (!amount || !fromAccount || !toAccount) {
      return { speakableText: "The transfer has been completed successfully." };
    }

    return { 
      speakableText: "The transfer has been completed successfully. Here are the details.",
      confidentialData: {
