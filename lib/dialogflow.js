  ...
  try {
    if (process.env.GOOGLE_CREDENTIALS_JSON) {
      ...
      sessionClient = new dialogflow.SessionsClient({
        ...
        if (!credentialsPath) {
            throw new Error("Google credentials are not set. Please set either GOOGLE_CREDENTIALS_JSON or GOOGLE_APPLICATION_CREDENTIALS.");
        }
        sessionClient = new dialogflow.SessionsClient({
            projectId: projectId,
...
async function handleAgentHandoff() {
  return { 
    speakableText: "I understand. Let me connect you to a live agent.",
