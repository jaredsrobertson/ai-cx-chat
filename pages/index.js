  ...
  useEffect(() => {
    const handleScroll = () => {
      ...
      sections.forEach((section) => {
        ...
        // Distance for fade zone at edges
        const fadeDistance = containerHeight * 0.2; // 20% of viewport for fade zone
        
        let opacity = 1;
        
        // Fade when section is exiting through the TOP (scrolling down)
        if (sectionTop < viewportTop) {
          const distanceAboveViewport = viewportTop - sectionTop;
          if (distanceAboveViewport < fadeDistance) {
            opacity = Math.min(opacity, 1 - (distanceAboveViewport / fadeDistance));
          } else {
            opacity = 0; // Completely out of view
          }
        }
        
        // Fade when section is exiting through the BOTTOM (scrolling up)
        if (sectionBottom > viewportBottom) {
          const distanceBelowViewport = sectionBottom - viewportBottom;
          if (distanceBelowViewport < fadeDistance) {
            opacity = Math.min(opacity, 1 - (distanceBelowViewport / fadeDistance));
          } else {
            opacity = 0; // Completely out of view
          }
        }
        ...
        const content = section.querySelector('.fade-content');
        if (content) {
          content.style.opacity = opacity;
          content.style.transform = `translateY(${(1 - opacity) * 20}px)`;
        }
      });
    };
              ...
              <div className="flex justify-between items-center h-16">
                <div className="flex items-center space-x-4">
                  <div className="w-10 h-10 bg-white rounded-lg flex items-center justify-center shadow-md">
                    <CloudIcon className="w-6 h-6 text-brand-blue" />
                  </div>
              ...
              <div className="fade-content">
                <div className="max-w-7xl w-full mx-auto px-4 sm:px-6 lg:px-8">
                  <div className="text-center">
                    <h2 className="text-6xl font-bold mb-6 leading-tight text-white">Dual-Mode AI Assistant</h2>
                ...
                <div className="w-full max-w-5xl mx-auto px-4 sm:px-6 lg:px-8">
                  <div className="card">
                    ...
                    <div className="grid md:grid-cols-3 gap-8">
                      <div className="text-center">
                        <h4 className="text-xl font-bold text-gray-900 mb-2">Google Dialogflow</h4>
                        <p className="text-gray-600">For NLU and stateful, task-oriented dialogue.</p>
                      </div>
                      <div className="text-center">
                        <h4 className="text-xl font-bold text-gray-900 mb-2">OpenAI GPT-4</h4>
                        <p className="text-gray-600">For generative, open-ended conversational ability.</p>
                      </div>
                      <div className="text-center">
                        <h4 className="text-xl font-bold text-gray-900 mb-2">Full-Stack Next.js</h4>
                        <p className="text-gray-600">With serverless API routes, JWT auth, and Vercel KV rate-limiting.</p>
                      </div>
                    </div>
                  </div>
                </div>
